...
import { createTest } from "../components/ApiUtilsi";
...
const TestPage = ({ navigation }) => {
  ...
  const [improvementRecommendation, setImprovementRecommendation] = useState(null);
        ...
        <View style={styles.answerContainer}>
          {["전혀 아니다", "아니다", "그렇다", "매우 그렇다"].map(
            (option, index) => (
              <TouchableOpacity
              key={index}
              style={[styles.answerButton, answers[questionNumber - 1] === index ? styles.selectedAnswer : null]}
              onPress={() => {
  ...
  // 각 점수 계산
  const metaCognitionScore = calculateCategoryScore([0, 1, 2, 3]) + calculateCategoryScore([4, 5]) + calculateCategoryScore([6, 7]);
  const monitoringScore = calculateCategoryScore([8, 9]) * 2 + calculateCategoryScore([10, 11]) * 2;
  const metaControlScore = calculateCategoryScore([12, 13]) + calculateCategoryScore([14, 15]) + calculateCategoryScore([16, 17]) + calculateCategoryScore([18, 19]);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView ref={scrollViewRef} contentContainerStyle={styles.scrollViewContent}>
        {Array.from({ length: 5 }, (_, i) => renderQuestion(i))} {/* 각 페이지당 5개 질문 */}
      </ScrollView>
      ...
      {resultImage && (
        <View style={styles.resultContainer}>
          ...
          <Text style={styles.recommendationText}>{improvementRecommendation}</Text>
        </View>
      )}
    </View>
  );
