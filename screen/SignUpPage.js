...
export default function SignUpPage({ navigation }) {
  ...
  const checkDuplicateId = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/members/check?username=${username}`);
      if (response.ok) {
  ...
  const handleSignUp = async () => {
    console.log("회원가입 시도: ", { username, name, password, confirmPassword });
    ...
    if (isDuplicate) {
      setModalMessage("이미 사용 중인 아이디입니다. 다른 아이디를 사용해 주세요.");
      setModalVisible(true);
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        style={[
          styles.input,
          !isPasswordMatch && confirmPassword.length > 0 ? styles.errorInput : {},
      ...
      <TouchableOpacity 
        ...
        style={[styles.signUpButton, password.length < 6 ? styles.disabledButton : {}]} // 비밀번호가 6자리 미만일 경우 버튼 비활성화
